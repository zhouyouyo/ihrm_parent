...
public interface UserCompanyPersonalDao extends JpaRepository<UserCompanyPersonal, String>, JpaSpecificationExecutor<UserCompanyPersonal> {
    ...
    /*@Query(value = "select new com.ihrm.domain.employee.response.EmployeeReportResult(a,b) from UserCompanyPersonal a left join EmployeeResignation b" +
            " on a.userId = b.userId where a.companyId = ?2 and a.timeOfEntry like ?1 or " +
            "b.resignationTime like ?1")
    public List<EmployeeReportResult> findByReport(String month, String companyId);*/
